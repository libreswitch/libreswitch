SUMMARY = "OpenVSwitch for LibreSwitch"
LICENSE = "MIT"
...
inherit libreswitch autotools distutils pkgconfig systemd useradd
...
do_install() {
    ...
    echo "d /run/openvswitch/ 0770 - ovsdb-client -" > ${D}${sysconfdir}/tmpfiles.d/libreswitch.conf
    echo "a+ /run/log/journal/%m - - - - d:group:ops_netop:r-x" >> ${D}${sysconfdir}/tmpfiles.d/libreswitch.conf
    echo "A+ /run/log/journal/%m - - - - group:ops_netop:r-x" >> ${D}${sysconfdir}/tmpfiles.d/libreswitch.conf
...
FILES_ops-ovsdb = "/run /var/run /var/log /var/volatile ${bindir}/ovsdb* \
  ...
  ${sysconfdir}/tmpfiles.d/libreswitch.conf"
