SUMMARY = "OpenVSwitch for Halon"
LICENSE = "MIT"
LIC_FILES_CHKSUM = "file://${COMMON_LICENSE_DIR}/MIT;md5=0835ade698e0bcf8506ecda2f7b4f302"

DEPENDS = "openssl python perl systemd halonutils libtool"
...
PACKAGES =+ "halon-ovsdb python-halon-ovsdb"
PROVIDES = "${PACKAGES}"

RPROVIDES_${PN} = "virtual/switchd"

RDEPENDS_${PN} = "openssl procps util-linux-uuidgen util-linux-libuuid coreutils \
  python perl perl-module-strict sed gawk grep halon-ovsdb \
  ${@bb.utils.contains('MACHINE_FEATURES', 'halonsim', 'openvswitch-sim-switch', '',d)} \
"

RDEPENDS_python-halon-ovsdb = "python-io python-netclient python-datetime \
...
EXTRA_OECONF += "TARGET_PYTHON=${bindir}/python \
                 ...
                 ${@bb.utils.contains('MACHINE_FEATURES', 'halonsim', '--enable-simulator-provider', '',d)} \
                 "
...
do_install_append() {
    ...
    if ${@bb.utils.contains('MACHINE_FEATURES','halonsim','true','false',d)}; then
...
SYSTEMD_PACKAGES = "${PN} halon-ovsdb"

SYSTEMD_SERVICE_${PN} += "switchd.service"
SYSTEMD_SERVICE_halon-ovsdb = "ovsdb-server.service"
